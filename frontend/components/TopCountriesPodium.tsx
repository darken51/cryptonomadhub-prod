...
function TopCountriesPodium({ countries }: TopCountriesPodiumProps) {
  ...
  const OlympicPodium = ({ countries }: { countries: CountryScore[] }) => {
    ...
    // Reorder: [2nd, 1st, 3rd] for podium display (desktop only)
    const podiumOrder = [countries[1], countries[0], countries[2]]
    const positions = [1, 0, 2]

    return (
      <>
        {/* Mobile: Vertical Stack (1st, 2nd, 3rd in order) */}
        <div className="flex md:hidden flex-col gap-4 mb-8">
          {countries.map((country, idx) => (
            ...
            >
              <div className={`bg-gradient-to-r ${getPodiumBg(idx)} rounded-xl p-4 shadow-lg`}>
                <div className="flex items-center justify-between text-white">
                  <div className="flex items-center gap-3">
                    <div className="text-4xl">{getMedalIcon(idx)}</div>
                    {country.flag_emoji && (
                      <div className="text-4xl">{country.flag_emoji}</div>
                    )}
                    <div>
                      <div className="font-bold text-xl">{country.country_name}</div>
                      <div className="text-sm opacity-90">{country.country_code}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-4xl font-bold">{country.score}</div>
